...
import {FindTransectionsQueryDto, UserHomeQueryDto} from './dtos/find-transections.query.dto';
...
export class UserController {
    ...
    async findAllSuggestedFriends(@GetUser() user: UserDocument, @Query() {limit, page}: PaginationDto) {
        ...
        const randomUsers=await this.usersService.findRandomResult(allUsers.concat(userFriends),user._id,parseInt(limit) || 10);
    ...
    @Get('home')
    async home(@GetUser() user: UserDocument, @Query() {pageId}: UserHomeQueryDto) {
    ...
    async approveRejectSeller(@Body() {sellerRequest, user}: ApproveRejectSellerDto) {
        ...
        if (sellerRequest === SellerRequest.APPROVED) {
            ...
            await this.usersService.findOneRecordAndUpdate({_id: user}, {
                sellerRequest: SellerRequest.REJECTED,
                $unset: {sellerId: 1}
            });
        }
