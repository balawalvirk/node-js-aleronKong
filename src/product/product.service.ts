...
export class ProductService extends BaseService<ProductDocument> {
    ...
    async findStoreProducts(query: FilterQuery<ProductDocument>, options?: QueryOptions<ProductDocument>) {
        return await this.productModel.aggregate([
            {$match: query},
            {
                $sort: (!options?.sort?.before && !options?.sort) ? {createdAt: -1} : (options?.sort?.before || options?.sort),
            },
            {
                $lookup: {
            ...
            {
                $sort: (!options?.sort?.after && !options?.sort) ? {"products.createdAt": -1} : (options?.sort?.after || options?.sort),
            },
            {
                $project: {
                    _id: 0,
