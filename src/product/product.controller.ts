...
export class ProductController {
    ...
    async create(@Body() createProductDto: CreateProductDto, @GetUser() user: UserDocument) {
        if (createProductDto.webSeries) {
            createProductDto.series = createProductDto.series.map((series) => (series.price === 0 ? {
                ...series,
                isFree: true
            } : series));
        }
        ...
        return await this.productService.create(
            createProductDto.price === 0 ? {
                ...
                isFree: true,
                creator: user._id
            } : {...createProductDto, creator: user._id}
        );
    }
    ...
    async createReview(@GetUser() user: UserDocument, @Body() createReviewDto: CreateReviewDto) {
        ...
        await this.productService.findOneRecordAndUpdate({_id: createReviewDto.product}, {
