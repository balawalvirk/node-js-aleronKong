...
export class FundraisingController {
    ...
    async fundProject(@Body() {amount, projectId}: FundProjectDto, @GetUser() user: UserDocument) {
        ...
        const project = await this.fundraisingService.findOneRecordAndUpdate({_id: projectId}, {
            $inc: {
        ...
        if (post.creator.fcmToken && post.creator.enableNotifications) {
    ...
    async approve(@Param('id', ParseObjectId) id: string, @GetUser() user: UserDocument) {
        ...
        if (project.creator.fcmToken && project.creator.enableNotifications) {
            await this.firebaseService.sendNotification({
