...
export class PackageController {
    ...
    async update(@Param('id') id: string, @Body() updatePackageDto: UpdatePackageDto, @GetUser() user: UserDocument) {
        ...
        if (packageFound.price === updatePackageDto.price) {
            ...
            return await this.packageService.findOneRecordAndUpdate({_id: packageFound._id}, {
                ...updatePackageDto,
                priceId: price.id
            });
        }
    ...
    async subscribe(@GetUser() user: UserDocument, @Param('id', ParseObjectId) id: string) {
        ...
        const pkgExists = pkg.buyers.find((buyer) => buyer.user == user._id);
        ...
        if (!pkg.isGuildPackage) {
            const userFound = await this.userService.findOneRecord({_id: user._id}).populate({
                path: 'supportingPackages.package',
                select: 'creator'
            });
        ...
        if (pkg.isGuildPackage) {
            await this.userService.findOneRecordAndUpdate({_id: user._id}, {
                $push: {supportingPackages: {package:pkg._id}},
                isGuildMember: true
            });
        } else {
            await this.userService.findOneRecordAndUpdate({_id: user._id}, {$push: {supportingPackages: {package:pkg._id}}});
        }
    ...
    async remove(@Param('id', ParseObjectId) id: string, @GetUser() user: UserDocument) {
        ...
        await this.userService.updateManyRecords({supportingPackages: {$in: [{package:pkg._id}]}},
            {$pull: {supportingPackages: {package:pkg._id}}});
    ...
    async unSubscribePackage(@Param('id', ParseObjectId) id: string, @GetUser() user) {
        ...
        const pkgExists = user.supportingPackages.find((pkg) => pkg.package && pkg.package.equals(id));
        ...
        if (pkg.isGuildPackage) {
            ...
            await this.userService.findOneRecordAndUpdate({_id: user._id}, {$pull: {supportingPackages: {package:pkg._id}}});
        }
        await this.packageService.findOneRecordAndUpdate({_id: id}, {$pull: {buyers: {user:user._id}}});
    ...
    async findMembership(@GetUser() user: UserDocument, @Param('id', ParseObjectId) id: string) {
        ...
        const invoices = allInvoices.data.map((invoice) => ({
