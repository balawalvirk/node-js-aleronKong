...
class BaseService {
    constructor(model) {
        ...
        this.findAllRecords = (filter) => this.model.find(filter);
        this.findOneRecord = (filter) => this.model.findOne(filter).lean();
        this.paginate = (condition, paginate) => this.model.find(condition, {}, paginate);
        this.countRecords = (filter) => this.model.countDocuments(filter);
        this.findRecordById = (id) => this.model.findById(id).lean();
